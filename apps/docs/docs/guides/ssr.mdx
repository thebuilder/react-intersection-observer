...
- **Server rendering** needs markup before a browser exists. `initialInView` does not create an observer or guarantee that the target is visible.
- **Hydration** attaches the observer after the browser takes over. The first client notification can change the state.
- **Unsupported clients** have a browser environment but no `IntersectionObserver` implementation. A client without IntersectionObserver throws unless a fallback is configured.
- **Fallback scope** is deliberate: fallbackInView is the local fallback and defaultFallbackInView is the global policy.
